...
class UsersAjaxAPI extends AjaxController {
    ...
    function manage($id, $target=null) {
        ...
        if($_POST){ //funciona qnd chega dados - verifica se vem dados do form
            // print("Dentro do if");
            // print_r($account);
            // die();
            if ($account->update($_POST, $errors)) //executa update, senao pega erros
            ...
            if ($errors['err']) 
                $info['error'] = $errors['err'];
            else if($errors['user_invalidate']){ //INSERIR MAIS UM ELSE IF DE TRATAMENTO DE ERRO
                $info['error'] = __('Impossível atualizar usuário.') //AQUI ERRO DO USERNAE
                    .' '.__('Nome de usuário já existe.');
            }
            ...
            // print_r($errors);
            // print_r($errors);
                $info['error'] = __('Impossível atualizar conta.') //AQUI ERRO DO USERNAE
                    .' '.__('Corrija os erros abaixo e tente novamente.');
        }
